...
import cn.freefly.springboot.httpClientUtil.dto.gj.financeInfo.*;
...
public class FinanceInfoInputServiceImpl implements FinanceInfoInputService {
    ...
    public BaseResponse saveFinanceInfo(ChannelFinanceInfoInputRequest infoInputRequest) {
        ...
        if(!statusCodeList.contains(order.getStatusCode())){
            ...
            return BaseResponse.createBaseResponse(ResFlagE.失败.isFlag(),ResponseCode.CHANNEL_FINANACE_INFO_INPUT_NOT_APPLY);
        }
        // 查询资方配置信息
        ...
        //2、组装请求资方参数
        FinanceInfoInputRequest financeInfoInputRequest = buildFinanceInfoInputRequestModel(infoInputRequest,order.getInstNo());
        if (Objects.isNull(financeInfoInputRequest)) {
            log.info("融资信息录入接口申请缺少影像资料");
            return BaseResponse.createBaseResponse(ResFlagE.失败.isFlag(),ResponseCode.CHANNEL_FINANACE_INFO_INPUT_ATTACH_NOT_EXIST);
        }

        //3、资方接口调用
        BaseResponse response = gjHttpClient.post(fundService.getSaveApiUrl(), financeInfoInputRequest, applicationName, fundService);
    ...
    private FinanceInfoInputRequest buildFinanceInfoInputRequestModel(ChannelFinanceInfoInputRequest infoInputRequest,String instNo) {
        ...
        // 查询订单当前节点
        financeInfoInputRequest.setInstNo(instNo);
        financeInfoInputRequest.setNodeNo(EnumProcessNode.FINANACE_INFO_INPUT.getCode());
        //资产汽车表
        ...
        Optional<CacmpGjFile> cardYouhouFic = cacmpGjFiles.stream().filter(v -> Objects.equals(v.getFileName(), GJFieldEnum.右后45度.name())).findFirst();
        if (cardYouhouFic.isPresent()) {
            financeInfoAssetCar.setYouhouFic(cardYouhouFic.get().getFastdfsUrl());
        }
        ...
        if(channelFinanceInfoAssetRealEstateList.size() > 0){
            for(ChannelFinanceInfoAssetRealEstate channelFinanceInfoAssetRealEstate:channelFinanceInfoAssetRealEstateList){
                ...
                financeInfoAssetRealEstate.setCondition(channelFinanceInfoAssetRealEstate.getConditionState());
